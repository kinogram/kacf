...
fn api_key_secret_path() -> PathBuf {
    managed_root_path().join(API_KEY_SECRET_FILENAME)
}
...
fn random_bytes(len: usize) -> Vec<u8> {
    ...
    let mut x = now_unix() ^ 0x9e37_79b9_7f4a_7c15;
    for b in &mut out {
        x ^= x << 7;
        x ^= x >> 9;
        x ^= x << 8;
...
fn load_or_create_api_key_secret() -> std::io::Result<Vec<u8>> {
...
fn encrypt_api_key(plain: &str) -> Option<String> {
    if plain.trim().is_empty() {
        return Some(String::new());
    }
    let secret = load_or_create_api_key_secret().ok()?;
...
fn decrypt_api_key(enc: &str) -> Option<String> {
...
fn encrypt_shared_config_for_storage(cfg: &mut SharedConfig) {
    if !cfg.encrypt_api_key {
        if is_encrypted_api_key(&cfg.api_key) {
            if let Some(plain) = decrypt_api_key(&cfg.api_key) {
                cfg.api_key = plain;
            }
        }
        return;
    ...
    if let Some(enc) = encrypt_api_key(&cfg.api_key) {
        cfg.api_key = enc;
    } else {
...
fn list_language_packs() -> Vec<String> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(ok_code) = sanitize_language_code(code) {
            items.push(ok_code);
        }
    }
    items.sort();
...
async fn get_language_pack(path: web::Path<String>) -> impl Responder {
    ...
    match read_language_pack(&code) {
        Some(content) => HttpResponse::Ok()
            ...
            .body(content),
        None => HttpResponse::NotFound().body("language not found"),
    }
}
